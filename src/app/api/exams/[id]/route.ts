...
export async function GET(
  ...
  try {
    ...
    const responseExam = {
      ...exam,
      password: exam.isPasswordProtected && (user.role === 'ADMIN' || user.role === 'MODERATOR' || exam.createdById === userId) 
        ? exam.password 
        : exam.isPasswordProtected ? '[PROTECTED]' : undefined,
...
export async function PUT(
  ...
  try {
    ...
    if (user.role !== 'ADMIN' && user.role !== 'MODERATOR' && existingExam.createdById !== userId) {
      return NextResponse.json({ error: 'Insufficient permissions to update this exam' }, { status: 403 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (user.role !== 'ADMIN' && exam.createdById !== userId) {
      return NextResponse.json({ error: 'Insufficient permissions to delete this exam' }, { status: 403 });
    }
